<x-admin.layout>
    <x-slot name="transactioncancel_list">
    <div class="container-fluid">
        <div class="d-flex gap-5 mt-5 justify-content-start" style="min-width:720px;">
            <div class="rounded shadow-lg bg-opacity-75 d-flex align-items-center px-4 justify-content-between" style="background-color:#330000; width:250px;height:110px;min-width:220px;">
                <div class="d-flex flex-column">
                    <small class="p-0 m-0 text-white">Ongoing Borrow</small>
                    <h6 class="p-0 m-0 fw-bold fs-4 text-white">{{ $cancel }}</h6>
                </div>
        ...
        <form class="d-flex form-group mt-5 px-5" action="{{ route('admin.transactioncancel.list')}}" method="GET">
                ...
                @foreach ($transactions as $approved )
                    <tr>
                        @if($approved->status == 'cancel')
                            ...
                            <td class="align-middle fs-6 text-danger">{{ $approved->status }}</td>
